...
(defun quality-pc-set (quality-string)
  (second (find-if #'(lambda (q)
                       (string-equal (first q) quality-string))
          chord-qualities)))

(defun chord-pcs (pc-root quality-string)
  (set-transpose (quality-pc-set quality-string) pc-root))
...
;(defmethod only-complete-chords ((chord chord) chord-list)
 ; (remove-if-not #'(lambda (c)
  ;                   (complete-p c (chord-pcs (root chord) (quality chord))))
   ;              chord-list))
